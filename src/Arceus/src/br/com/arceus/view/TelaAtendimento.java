...
public class TelaAtendimento extends JFrame {
	...
	private int cod = gc.getCodigo(
			"select id_projeto from atender order by id_projeto desc limit 1",
			"ID_PROJETO");
	...
	public TelaAtendimento() {
		...
		btnNovo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					if (cliente.equalsIgnoreCase("fisico")) {
						gc.dadosCliente(
								"SELECT * from clientefisico where cpf = ?",
								null,null, campoNomeCliente.getText().toString()
										.trim(), tabelaCliente);

						tabelaCliente.selectAll();
						codigo = (String) tabelaCliente.getModel().getValueAt(
								tabelaCliente.getSelectedRow(), 0);

						gc.exibir("Select * from pessoa where id_pessoa = ?",
								codigo, tabelaCliente);

					} else {
						gc.dadosCliente(null,
								"SELECT * from clientejuridico where cnpj = ?",null,
					...
					JOptionPane.showMessageDialog(null,
							"Este cliente não esta cadastrado !!!!");
				}
		...
		botaoOrcamento.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				texto = campoDescricaoProjeto.getText();
				//gc.cadastrar(
				//		"INSERT into atendimento (ORCAMENTO, CADASTRO_PEDIDO, ID_SETOR, ID_PESSOA) VALUES(?,?,?,?)",
				//		"" + campoValorProjeto.getText().toString().trim()
					//			+ "#SIM#4#" + codigo + "");

				// REMOVERDO BANCO O ID_SETOR!!!!!!!!!!!!!!!!!!!!!!!!!!!!1

				gc.cadastrar(
						"INSERT into atender (DATA_ENTREGA, DATA_PEDIDO, VALOR_PEDIDO, DESCRICAO_PROJETO,ID_PROJETO, ID_PESSOA) VALUES(?,?,?,?,?,?)",
						"" + campoDataEntrega.getText().toString().trim() + "#"
								+ campoDataEmissao.getText().toString().trim()
								+ "#"
								+ campoValorProjeto.getText().toString().trim()
								+ "#" + texto + "#" + campoCodigoProjeto.getText().toString().trim() + "#" + codigo.trim()
								+ "");

				campoCodigoProjeto.setText((Integer.parseInt(campoCodigoProjeto
						.getText().toString()) + 1) + ""); // atualiza o codigo
				
				JOptionPane.showMessageDialog(null, "Pedido enviado ao setor financeiro!!");
		...
		campoCodigoProjeto.setText(cod + "");
